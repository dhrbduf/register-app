pipeline {
	...
	stage("Build & Push Docker Iamge"){
       ...
       stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user admin:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'http://3.36.122.57:8080/job/gitops-register-app-cd/buildWithParameters?token=gitops-token'"
                }
            }
       }
